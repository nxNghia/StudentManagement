...
const getAll = async (filter = {type: 'id', order: 'ASC'}) => {
    const result = await pool.query(`SELECT * FROM Subject ORDER BY ${filter.type} ${filter.order}`);
...
const add = async (data) => {
    const result = await pool.query(`INSERT INTO Subject (name, faculty, classes, credit) 
