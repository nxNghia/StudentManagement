...
const LogIn = async (req, res) => {
  ...
  try {
    ...
    if (user.rows[0]) {
      ...
      if (valid) {
        res.cookie('user', {id: user.rows[0].id, type: 'student'})
        res.status(200).send({ message: "Student login", user: user.rows[0]});
      } else {
        ...
        if (user.rows[0]) {
          const valid = await bcrypt.compare(password, user.rows[0].password)
          if (valid) {
            res.cookie('user', {id: user.rows[0].id, type: 'admin'})
            res.status(200).send({ message: "Admin login", user: user.rows[0]});
          } else {
            res.status(400).send({ message: "Wrong password!", email: email });
          }
        } else {
